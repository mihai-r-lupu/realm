...
describe('listRuns', () => {
    ...
    it('counts retried steps as one step', async () => {
        const run = makeRun({
            evidence: [
                { step_id: 'step_one', kind: 'execution', started_at: '', completed_at: '', duration_ms: 1, input_summary: {}, output_summary: {}, status: 'error', evidence_hash: 'a' },
                { step_id: 'step_one', kind: 'execution', started_at: '', completed_at: '', duration_ms: 1, input_summary: {}, output_summary: {}, status: 'success', evidence_hash: 'b' },
                { step_id: 'step_two', kind: 'execution', started_at: '', completed_at: '', duration_ms: 1, input_summary: {}, output_summary: {}, status: 'success', evidence_hash: 'c' },
            ],
        });
        const result = await listRuns(undefined, makeStore([run]));
