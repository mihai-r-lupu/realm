...
| `tools`           | `string[]`                      | No       | Tool names this step may call, in `server_id:tool_name` format. Only valid on `execution: agent` steps with an `input_schema`. References entries in `mcp_servers`. All declared names must exist in the connected server — if any name is not found after `listTools()`, the step fails immediately with `MCP_TOOL_NOT_FOUND` before any LLM call is made.                                                                                              |
