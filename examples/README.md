...
| [09-webhook-pr-review/](09-webhook-pr-review/)       | End-to-end webhook pipeline with no structural boundary between MCP tool calls, Slack notification, human review, and GitHub write | `realm webhook` receives the event, MCP agent steps fetch and analyze, Slack adapter fires before gate opens, `when` condition blocks the GitHub write until gate resolves | ⏸     |
