...
## Phase 3: Second Workflow, Branching, Templates (Weeks 9-11)

### Week 9-10 — Second Workflow + Branching (20-30 hrs)

**Goal:** Build a second workflow in a different domain to prove generality. Identify and fix abstraction gaps.
...
1. **Second workflow YAML + schema** — complete workflow in a different domain.

2. **Conditional branching** — `transitions` with `on_success`, `on_error`, `on_confirm`, `on_cancel`. Condition expressions evaluated by the expression evaluator.

3. **`input_map` with expression language** — steps receive computed inputs from prior step results. Expression evaluator extended from preconditions to handle property access, pipe filters, ternary.

4. **`on_error: fallback`** — error recovery routes to alternative step.

5. **Any missing adapters or processors** discovered while building the second workflow.
...
**Milestone: Phase 3 complete. Two workflows in different domains. Shared templates. Conditional branching. Expression language for input transforms.**
...
3. **Blog post:** "How I made AI agents follow a 12-step process without going off-script"
...
### Weeks 14-15 — Cloud API (20-30 hrs)

**Only proceed if Phase 4 launch shows signal (stars, installs, questions, interest).**
...
2. **HTTP API** — Express or Fastify server. Same 6 operations as MCP tools, exposed as REST endpoints. Auth via API keys.

3. **Deploy** — Railway or Fly.io. Postgres via Neon or Supabase.

4. **User registration** — sign up, get API key, register workflows via API.
...
| Competitor launches similar product | Market confusion | The evidence chain + quote verification is hard to replicate. Ship fast, build community. |
...
| 13 | Public launch | Repo public, npm published, blog post live |
| 15 | Cloud API works | Workflows run via HTTP API with Postgres store |
| 17 | Dashboard works | Web UI shows runs, evidence, workflow versions |
| 20 | Revenue | First paying customer on $49/month plan |
| 30 | Ecosystem | 5+ adapters, 3+ domain bundles, $5K MRR |
